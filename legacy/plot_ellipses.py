...
for SD_nparticle in SD_nparticles:
    data = np.loadtxt(os.path.join(plot_save_path, "SD_ellipses_nparticle{}".format(SD_nparticle)))
...
for FW_nparticle in FW_nparticles:
    data = np.loadtxt(os.path.join(plot_save_path, "FW_ellipses_nparticle{}".format(FW_nparticle)))
...
plt.ylabel('Sinkhorn barycenter loss', fontsize=18)
leg = plt.legend()
leg_texts = leg.get_texts()
plt.setp(leg_texts, fontsize='x-large')
plt.savefig(os.path.join(plot_save_path, 'ellipses.pdf'))
# plt.show()
